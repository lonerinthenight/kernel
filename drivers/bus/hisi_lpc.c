...
static int hisilpc_probe(struct platform_device *pdev)
{
	...
	io_ops = devm_kzalloc(&pdev->dev, sizeof(*io_ops),
				GFP_KERNEL);
	if (!io_ops)
		return -ENOMEM;
	 ...
	 */
	io_ops->start = 0;
	io_ops->end = PCIBIOS_MIN_IO - 1;
	io_ops->devpara = lpcdev;
	io_ops->pfin = hisilpc_comm_in;
	io_ops->pfout = hisilpc_comm_out;
	io_ops->pfins = hisilpc_comm_ins;
	io_ops->pfouts = hisilpc_comm_outs;

	console_lock();
	arm64_set_extops(io_ops);
	console_unlock();
	...
	if (!has_acpi_companion(&pdev->dev)) {
		...
		for_each_available_child_of_node(root, child) {
			dev_info(&pdev->dev, "create platform device for %s",
					child->name);
