 ...
 *
 */
static unsigned int __init hslpc_early_in(struct uart_port *port, int offset)
{
	...
	ret = hslpc_target_in(&lpcdev, &para,
 ...
 *
 */
static void __init hslpc_early_out(struct uart_port *port, int offset,
					int value)
{
	struct lpc_cycle_para para;
	struct hslpc_dev lpcdev;
	...
	(void)hslpc_target_out(&lpcdev, &para,
				port->iobase + (offset << port->regshift),
						...
						const char *options)
{
	...
	device->port.serial_in = hslpc_early_in;
	device->port.serial_out = hslpc_early_out;
